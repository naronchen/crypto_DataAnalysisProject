...
proc means data=fcmart (obs=5);
proc sgplot data= apemeans;
series x=targetyear y=apestats_median/group=model;
...
class productgroup launchmonth model targetcalmonth / param=effect show;
model apestatshift=prodctgroup|priceindex|launchmonth|producttag|
model|leadtime|targetcalmonth|targetyearshift @1 /
details=steps selection=stepwise (select=adjrsq) orderselect showpvalues;
proc surveyselect data=fcmart out=fcmartsample mehtod=srs sampsize=1000
seed=19416;
proc means data=fcmartsample q1;
class model;
var apestat;
proc quantselect data=fcmartsample;
class productgroup launchmodel model targetcalmonth /
param effect show;
run;
